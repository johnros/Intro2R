...
# SVM
library('e1071')
svm.fit <- svm(y=y.train, x=X.train, probability=TRUE)


 
#####step 2a: build predictions for data.train####
train.predict<- cbind(
  predict(logistic.2, newx=X.train, type="response"),
  predict(logistic.3, newx=X.train, type="response"),
  knn1(train=my.codebook$x, test=X.train, cl=my.codebook$cl),
  predict(svm.fit, X.train, probability=TRUE)
)

####step 2b: build predictions for data.test####
test.predict <- cbind(
  predict(logistic.2, newx=X.test, type="response"),
  predict(logistic.3, newx=X.test, type="response"),
  predict(svm.fit, newdata = X.test, probability = TRUE),
  knn1(train=my.codebook$x, test=X.test, cl=my.codebook$cl)
)

 
####step 3: train SVM on train.predict####
final <- svm(y=y.train, x=train.predict, probability=TRUE)
