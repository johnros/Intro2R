...
In FA (\S\ref{sec:factor_analysis}) we assumed that the data is a mixing ($\loadings$) of some low dimensional unobservable attributes ($\latent$), up to some personal non modeled attributes ($\varepsilon$): $X=AS+\varepsilon$.
We then assumed that the latent attributes are multivariate Gaussian and solved a likelihood problem.
In ICA, we do not assume a generative model.  
...
\begin{remark}[Classical and Stress MDS]
