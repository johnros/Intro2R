...
\begin{remark}[PCA and SVD]
The solution to PCA is typically presented in literature using a matrix decomposition called Singular Value Decomposition (SVD). \marginnote{SVD}
Using SVD facilitates the computation of the solution, and perhaps adds more insight into what PCA is doing. Because it is not an essential part of the PCA problem, but rather the means for the solution, we do not pursue this avenue herein, and refer the reader to Section 14.5.1 in \citep{hastie_elements_2003}.
\end{remark}
...
The fact that the linear-space embedding of the data depends only on one of these three, has laid a bridge between feature embedding, such as PCA, and \emph{graph embedding} methods such as MDS (\S\ref{sec:mds}).
...
We now prove that the PCA solution can be cast in terms of the empirical covariance between variables ($\hat{\Sigma}=X'X$), the covariance between individuals ($\similaritys=XX'$) or the Euclidean distances ($\dissimilaritys=\norm{x_i-x_j}$).
