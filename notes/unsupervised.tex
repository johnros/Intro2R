...
The $z_i$'s are set to minimize the some penalty for geometric deformation called the \emph{stress function}.
...
\begin{remark}[Non Linear Dimensionality Reduction]
	The term Non linear Dimensionality Deduction (NLDR) deals with the nature of the embedding operation, and not with the structure of the target manifold $\manifold$ (see also Remark \ref{remark:linear}).\marginnote{NLDR}
...
Estimation can then be done using maximum likelihood, or an information theoretic criterion called \emph{infomax}.\marginnote{Infomax}
