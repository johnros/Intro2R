...
In this text, we distinguish between OLS and Linear Regression. We refer to Linear Regression when we assume that the data generating process it actually $\y=\x\be+\varepsilon$, whereas in OLS we merely fit a linear function without claiming it is the \emph{true} relation between $\y$ and $\x$.
\end{remark}
...
Eq.(\ref{eq:lasso}) does not have a closed form solution for $\estim{\be}$ but the LARS algorithm, a quadratic programming algorithm, solves it efficiently.
...
The sparsity property can be demonstrated for the orthogonal design case ($X'X=I$), in which $\estim{\be}$ admits a closed form solution:
\begin{align}
...
\begin{align}
	\loss(y,x,\be) = -\log \left[ \Psi(x\be)^{y} (1-\Psi(x\be))^{1-y}  \right].
\end{align}
...
\begin{align*}
	g_m(x w_m):= \be_m \sigma(\al_0 + x \al_m )
\end{align*}
where only $\set{w_m}_{m=1}^M = \set{\be_m, \al_m}_{m=1}^M$ are learned from the data. 
...
\item[Loadings] The weights of each data point in each principal component.
...
We thus denote the encoding $\featureS \mapsto \manifold$ operator by $\encode$, a $\rank \times p$ matrix.
We also denote the decoding operator by $\manifold \mapsto \featureS$ by $\decode$, a $p \times \rank$ matrix. 
The equivalent of the predictor function ($\hyp$) in this case is thus the decoding of the encoding: $\hyp(x_i)=\decode\encode x_i$.
...
%As an OLS problem, we seek to predict $X$ with $X$, where $\be$ is not a $p \times 1$ vector, but rather a $p \times p$ matrix of rank $\rank$.
%X (X'X)^{-1} X'X
%
