...
Unlike Supervised Learning, in Unsupervised Learning there is not outcome variable. There is thus no notion of ``right'' and ``wrong''. We merely want to learn the joint distribution of the data, $X \in \featureS$, and represent it in some way we can understand. 
Describing the data via it's joint distribution would be nice, but would require many samples if $X$ has a high dimension (by high we mean $p>3$!).

For low dimensional data, aim at learning the joint distribution. 


For higher dimensions we need to set more modest goals:
\begin{description}
...
This goal is discussed in the present section.
...
\begin{align}
\label{eq:parzen}
	\hat{\density}(x):=\frac{1}{n \lambda} \sum_{i=1}^n \kernel_\lambda(x,x_i).
\end{align}
...
\subsection{Mixtures}
A way to deal with high dimension distributions, 
...
\subsection{In Supervised Learning}
[TODO]
...
\subsection{In unsupervised Learning}
[TODO]
