...
\begin{align}
	Err_\sample :=& \expect{\loss(Y,\hat{\hyp}(X))|\sample}, \label{eq:test_error} \\
	Err :=& \expect{\loss(Y,\hat{\hyp}(X))}, \label{eq:expected_test_error} \\
	\overline{err} :=& \frac{1}{n} \sum_i \loss(y_i,\hat{\hyp}(x_i)). \label{eq:training_error}
\end{align}
...
It has been stated for completeness, but we will not study it herein.
...
The red curve is Figure~\ref{fig:bias_variance} depicts $Err_\sample $ while the blue depicts $\overline{err}$.
