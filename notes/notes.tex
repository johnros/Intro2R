...
\newcommand{\test}{\risk(\hat{\hyp})}
\newcommand{\train}{\riskn(\hat{\hyp})}
\newcommand{\insample}{\bar{\risk}(\hat{\hyp})}
\newcommand{\optimism}{op}
...
\begin{align}
	\test :=& \expect{\loss(Y,\hat{\hyp}(X))}, \label{eq:test_error} \\
	\train :=& \expectn{\loss(y,\hat{\hyp}(x))} = \frac{1}{n} \sum_i \loss(y_i,\hat{\hyp}(x_i)),  \label{eq:training_error} \\
	\insample :=& \frac{1}{n} \sum_i \expect{\loss(Y,\hat{\hyp}(x_i))}. \label{eq:in_sample}
\end{align}
...
It is known as the \emph{test error} or also as \emph{prediction error}.\marginnote{Test Error \\ Prediction Error}
Note that although $\hat{\hyp}(X)$ is learned from the data, it is \emph{not} treated as random when assessing its accuracy.


Eq.(\ref{eq:in_sample}) is the average risk at the observed $x$'s, when only $Y$ is treated as random.
This magnitude should not be unfamiliar: e.g., inference in ANOVA is performed conditional on the $x$'s, which typically stem from a designed experiment. 
...
If data is abundant, a trivial, assumption free way to go about, is to split the data into $3$ sets.
...
A \emph{test set}, used to estimate the risk, as the empirical risk will be unbiased $\train$ to the population risk $\test$.
...
Akaike's Information Criterion (AIC), the finite sample corrected AIC (AICc), Mallow's Cp (Cp), the Bayesian Information Criterion (BIC, aka SBC, aka SBIC), the Minimal Description Length (MDL), Vapnic's Structural Risk Minimization (SRM), the Deviance Information Criterion (DIC), the Focused Information Criterion (FIC), and the Hannan-Quinn Information Criterion (HQC), all try to estimate $\insample$ or $\test$ under different assumptions.\marginnote{AIC, BIC}
...
\paragraph{Further Reading}
For a brief review of AIC, BIC and SRM see Chapter 7 in \citep{hastie_elements_2003}. 
