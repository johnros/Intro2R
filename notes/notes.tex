...
\begin{align}
	\risk(\hat{\hyp}) :=& \expect{\loss(Y,\hat{\hyp}(X))}, \label{eq:test_error} \\
	\riskn(\hat{\hyp}):=& \frac{1}{n} \sum_i \loss(y_i,\hat{\hyp}(x_i)). \label{eq:training_error}
\end{align}
...
It is thus the same as estimating the expected height in a population, by using the minimum in each sample; we will clearly be underestimating the expectation. Then again, there is the hope that we may take this minimum and debias it. This is the goal in the next sections.

The first approach is analytic, discussed in Section~\ref{sec:risk_estimation} and requires some assumptions and some probability theory.
Other approaches are computational. They require less assumptions, but more computing power.
