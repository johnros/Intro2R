...
\EndFor
\State \Return the average loss of $\loss^{(i)}$.
\end{algorithmic}
...
\For {$k \in 1,\dots,\folds$}
    \State $\hat{\hyp}^{(k)} \gets$ the learned model with all \emph{except} the observations in the $\fold$'th fold.
    \State $\loss^{(\fold)} \gets$ the loss of $\hat{\hyp}^{(\fold)}$ on the observations in the $\fold$'th fold.
\EndFor
\State \Return the average of $\loss^{(\fold)}$ .
\end{algorithmic}
