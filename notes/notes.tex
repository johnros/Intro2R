...
Then $$\estim{\theta}_{ML}:= argmax_{\theta}\{ \lik(\theta) \}.$$
...
$\rv x_{t+1}=\rv x_t+\rv \varepsilon; \rv \varepsilon \sim \gauss{0,\sigma^2} \Rightarrow \rv x_{t+1}|\rv x_t \sim \gauss{a x_t,\sigma^2}$.
...
$$
  ...
  \right).
$$
Differentiating with respect to $a$ and equating $0$ we get $\estim{a}_{ML}=\frac{\sum x_{t+1}x_{t}}{\sum x_t^2}$.
...
\begin{align}
	\estim{\hyp}(x) = \argmin{\hyp}{\sum_i \loss(y_i - \hyp(x_i))}.
\end{align}
...
\begin{align}
	\estim{\beta}_{j,LASSO} = sign(\beta_j) \left[|\estim{\beta}_{j,OLS}|-\frac{\lambda}{2} \right]_+.
\end{align}
...
Solving the same problem from Example~\ref{eg:OLS} by encoding $y$ as $\{0,1\}$ gives us the linear separating hyperplane $\plane: \{x:x\estim{\beta}_{OLS}=0.5\}$.
...
The magic continues! It turns out that the fitted values, $\{\estim{\hyp}(x_i)\}_{i=1}^n$ are linear in $\{y_i\}_{i=1}^n$. A fact that greatly facilitates the analysis of the statistical properties of these models.
...
\For{$ i \in 1,\dots,n$}
	...
	\EndFor
\EndFor
\State $\bar{\hyp}(x_i) \gets$ the OLS prediction using $\{ \estim{\hyp}^{(i)}_\ensembleInd(x_i)\}_{\ensembleInd=1}^\ensembleSize$ as features. 
\State \Return $\bar{\hyp}(x)$
\end{algorithmic}
\end{algorithm}
...
\item[$\rv{x}$] A (possibly) vector valued random variable.
\item[$X$] A matrix.
\item[$\rv{X}$]  A matrix valued random variable (a random matrix).
\item[$X'$] The matrix transpose of $X$.
\item[$\norm{x}_2$] The $l_2$ norm of $x$: $\sqrt{\sum_j x_j^2}$.
\item[$\norm{x}_1$] The $l_1$ norm of $x$: $\sum_j |x_j|$
\item[$\sample$] A data sample.
\item[$\expect{x}$] The expectation of $\rv{x}$.
\item[$\expectn{x}$] The empirical expectation (average) of the vector $x$.
\item[$\cdf{x}{t}$] The CDF of $\rv{x}$ at $t$.
...
\item[$\rv x \sim \dist$]  The random variable $\rv{x}$ is $\dist$ distributed. 
