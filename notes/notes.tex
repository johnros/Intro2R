...
The penalty for predicting $\theta$ when observing $x$: $$\loss(x;\theta).$$
\end{definition}
\begin{definition}[Risk Function]
The expected loss: $$\risk(\theta):=\expect{\loss(\x;\theta)}.$$
\end{definition}
...
The average loss in the sample: $$\riskn(\theta):=\expectn{\loss(X;\theta)}=\frac{1}{n}\sum_i \loss(x_i,\theta).$$
\end{definition}
...
Computing the risk, $\expect{\frac{1}{2} \normII{\y-\x\beta}^2}$ will require dealing with the joint distribution of $(\x,\y)$.
...
\begin{align*}
	\riskn(\beta)=\sum_{i=1}^n 	\frac{1}{2}(y_i-x_i\beta)^2 = \frac{1}{2}\normII{y-x\beta}^2.
\end{align*}
...
Unlike classical statistics, we don't care to explain the causal process relating $x$ and $y$, we just want good predictions.  There is thus no need to assume the sampling distribution. The implied ERM problem is
\begin{align}
...
In this text, we distinguish between OLS and Linear Regression. We refer to Linear Regression when we assume that the data generating process it actually $\y=\x\beta+\varepsilon$, whereas in OLS we merely fit a linear function without claiming it is the \emph{true} relation between $\y$ and $\x$.
\end{remark}
...
\item[A Subjective Bayesian View] If we believe that $\beta$ should be small; say our beliefs can be quantified by $\rv \beta \sim \gauss{0,1/\lambda I}$, then the Ridge solution is actually the mean of our posterior beliefs on $\rv \beta|y$.
\end{description}

Whatever justification you prefer, it can be easily shown that $\deriv{\risk(\lambda,\beta)}{\lambda}$ at $\lambda=0$ is negative, thus, predictions only improve by introducing some small regularization (how small is small? see \S\ref{sec:desicion_theory}).
...
The LASSO has gained much popularity as it has the property that $\estim{\beta}_{LASSO}$ has many zero entries. It is thus said to be \emph{sparse}\marginnote{Sparsity}.
...
The sparsity property can be demonstrated for the orthogonal design case ($X'X=I$), in which $\estim{\beta}$ admits a closed form solution:
\begin{align}
...
\begin{align}
	...
	& \Psi(t) = \frac{1}{1+e^{-t}}
\end{align}
The hypothesis class is thus  $\hypclass=\set{\hyp:\hyp(x)=\Psi(x\beta)} $.
...
\begin{remark}[Not a pure ERM]
Just like the GAM problem, in the PPR problem $\set{g_m}_{m=1}^M$ are learned by kernel smoothing (\S\ref{sec:kernel}). Solving the PPR problem is thus a hybrid of ERM and Kernel smoothing algorithms.
\end{remark}
...
In the context of unsupervised learing, the empirical risk is know as the \emph{reconstruction error}.\marginnote{Reconstruction Error}
...
%\paragraph{Intuition}
%The main point to clarify is why is does $\estim{\projectMat_\rank}$ in Eq.(\ref{eq:PCA_solution}) solve Eq.(\ref{eq:PCA})?
%The intuition we offer is via OLS.
%As an OLS problem, we seek to predict $X$ with $X$, where $\beta$ is not a $p \times 1$ vector, but rather a $p \times p$ matrix of rank $\rank$.
%X (X'X)^{-1} X'X
%
%The intuition we offer is that because PCA is essentially a squared loss problem, it is solved by $\rank$ dimensional subspace that captures most of the variance in $X$. 
%The variance in $X$ is clearly $X'X$ so we would like a low dimensional representation of $X$, $\projectMat_\rank X$, that conserves $X'X$ as much as it can.
...
\item[$\icdf{x}{\alpha}$] The inverse CDF at $\alpha$ (the quantile function).
\item[$\cdfn{x}{t}$] The empirical CDF of data vector $x$.
\item[$\icdfn{x}{\alpha}$] The empirical $\alpha$ quantile of the data vector $x$.
