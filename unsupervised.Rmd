...
```{r PCA}
USArrests.1 <- USArrests[,-3] %>% scale # note the scaling, which is required by some functions down the road...
pca1  <-  prcomp(USArrests.1, scale. = TRUE) # The main workhorse.

pca1$rotation # loadings

# Now score the states:
USArrests.1[pca1$x %>% extract(,1) %>% which.max,] # Fewest arrests
USArrests.1[pca1$x %>% extract(,1) %>% which.min,] # Most arrests
