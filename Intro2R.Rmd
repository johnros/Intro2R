...
## magrittr: A slightly different style. Much better for nested expressions!
install.packages('magrittr') # download a package extension.
library(magrittr)

x %>% mean() %>% round(2) %>% runif  
x %>% var()
x %>% hist()  
...
### Import from WEB ###
tirgul1 <- read.table('http://statweb.stanford.edu/~tibs/ElemStatLearn/datasets/bone.data') # This is the main importing work-horse.
...
### Imprt .txt files ###
my.data<- read.table(file='C:\\Documents and Settings\\Jonathan\\My Documents\\...') # Tries to guess the seperator.
my.data<- read.delim(file='C:\\Documents and Settings\\Jonathan\\My Documents\\...') # Tab delimited.
...
# Matrix is more efficient. But can store only a single class.
